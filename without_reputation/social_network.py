...
from .prompt_template.run_gpt_prompt import (
    ...
    run_gpt_prompt_disconnection_after_pd_game_v1,
    run_gpt_prompt_connection_build_after_pd_game_v1,
)
...
def social_network_update(init_persona, target_persona, init_persona_role, target_persona_role, interaction_memory=None, full_investment=True):
    try:
        _ = init_persona.scratch.relationship["bind_list"].index(target_persona.scratch.name)

        if init_persona_role == "investor":
            disconnection_res = run_gpt_prompt_disconnection_investor_v1(init_persona, target_persona, interaction_memory)[0]
        elif init_persona_role == "trustee":
            disconnection_res = run_gpt_prompt_disconnection_trustee_v1(init_persona, target_persona, interaction_memory)[0]
        elif init_persona_role == "resident":
            disconnection_res = run_gpt_prompt_disconnection_after_chat_sign_up_v1(init_persona, target_persona, target_persona_role, interaction_memory)[0]
        elif init_persona_role == "player":
            disconnection_res = run_gpt_prompt_disconnection_after_pd_game_v1(init_persona, target_persona, interaction_memory)[0]
        else:
        ...
        if not full_investment:
            ...
            if init_persona_role == "investor":
                bind_res = run_gpt_prompt_connection_build_investor_v1(init_persona, target_persona, interaction_memory)[0]
            elif init_persona_role == "trustee":
                bind_res = run_gpt_prompt_connection_build_trustee_v1(init_persona, target_persona, interaction_memory)[0]
            elif init_persona_role == "resident":
                bind_res = run_gpt_prompt_connection_build_after_chat_sign_up_v1(init_persona, target_persona, target_persona_role, interaction_memory)[0]
            elif init_persona_role == "player":
                bind_res = run_gpt_prompt_connection_build_after_pd_game_v1(init_persona, target_persona, interaction_memory)[0]
            else:
                bind_res = "error"
