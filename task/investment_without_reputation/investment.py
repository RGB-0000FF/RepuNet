...
from without_reputation.prompt_template.run_gpt_prompt import run_gpt_prompt_update_learned_in_description_v1
...
def start_investment_without_reputation(pair, personas, G, save_folder):
    ...
    if "Refuse" in investor_decided:
        ...
        trustee_gossip_willing = run_gpt_prompt_stage4_trustee_gossip_v1(
            ...
            trustee_evaluation["investor_reflection"],
            verbose=True,
        )[0]
        investor_gossip_willing = run_gpt_prompt_stage4_investor_gossip_v1(
            ...
            trustee_allocation_part,
            investor_allocation_part,
            investor_evaluation["trustee_reflection"],
            verbose=True,
        )[0]
    ...
    if investor.scratch.complain_buffer:
        for person in investor.scratch.complain_buffer:
            if person["complaint_target_role"] == "trustee":
                ...
                gossip_target_investor = run_gpt_prompt_gossip_listener_select_v1(investor, "investor", personas[person["complaint_target"]])[0]
            else:
                gossip_target_investor = run_gpt_prompt_gossip_listener_select_v1(investor, "trustee", personas[person["complaint_target"]])[0]
    ...
    if trustee.scratch.complain_buffer:
        for person in trustee.scratch.complain_buffer:
            # gossip target choose
            if person["complaint_target_role"] == "trustee":
                gossip_target_trustee = run_gpt_prompt_gossip_listener_select_v1(trustee, "investor", personas[person["complaint_target"]])[0]
            else:
                gossip_target_trustee = run_gpt_prompt_gossip_listener_select_v1(trustee, "trustee", personas[person["complaint_target"]])[0]
